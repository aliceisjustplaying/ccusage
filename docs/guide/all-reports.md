...
`--by-provider` splits Pi-format sources into compact rows such as `Pi/Anthropic` and `Pi/OpenAI`. Provider rows include the same token, cache, model, and cost fields as agent rows; JSON output adds the exact provider ID in a `provider` field.
